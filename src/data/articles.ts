...
interface ComponentConstructorType {
  new (...args: any[]): {};
}

export function Repository<T>(option: RepositoryOption<T>) {
  function RepositoryDecorator<T1 extends ComponentConstructorType>(constructor: T1) {

    return class ExtendedRepository extends constructor {
      constructor(...args: any[]) {
        super(...args);


        const lifeCycles = ["onInit", "onDestroy"];
        for (const lifeCycle of lifeCycles) {
          const functionMaybe = (this as any)[lifeCycle];
...
@Repository({ dataSource: [], destination: "" })
export class Foo {
  constructor() {
  }
...
export class Bar {
  constructor() {
    console.log("-- exec Bar Constructor --");
  }
